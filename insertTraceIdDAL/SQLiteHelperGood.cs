...
namespace insertTraceId.DAL
{
    class SQLiteHelperGood
    {
        public class SQLiteHelper : System.IDisposable
        {
            ...
            #region 构造/析构函数
            /// <summary>
            /// 这个无参的构造方法继承与另外一个重载SQLiteHelper(string connectionstring)---通过参数类型判断重载哪个。
            /// 并对参数赋值(从配置文件中读取连接字符串)
            /// </summary>
            public SQLiteHelper()
                : this(ConfigurationManager.ConnectionStrings["conStr_sqlite"].ConnectionString)
            {
            }
            /// <summary>
            /// 初始化 SQLiteHelper
